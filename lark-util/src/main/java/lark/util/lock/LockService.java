...
public class LockService {
    ...
    public <T> T tryLock(String lockKey, int waitLockSeconds, int autoUnlockSeconds, ProcessHandle<T> handle) {
        ...
        try {
            ...
            log.error("acquire lock failure: {} waitLock:{} autoUnlock:{}, Cause:{}", lockKey, waitLockSeconds, autoUnlockSeconds, e.getMessage() );
        }
        if ( !canLock ) {
            log.error("acquire lock failure: {}, Processing... waitLock:{} autoUnlock:{}", lockKey, waitLockSeconds, autoUnlockSeconds );
